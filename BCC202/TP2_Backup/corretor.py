...
class Corrector(object):
    ...
    def __compare_outputs(self, file1, file2):
        ...
        data_file1 = [x for x in open(file1, 'r').read().replace(' ', '').split('\n\n') if x != '']
        data_file2 = [x for x in open(file2, 'r').read().replace(' ', '').split('\n\n') if x != '']
        if len(data_file1) != len(data_file2):
            return False
        for d1, d2 in zip(data_file1, data_file2):
            if d1 == '' and d2 == '':
                continue
            if len(d1) != len(d2):
                return False
